...
function CustomerApp({ inventory, refreshInventory, orders, refreshOrders, heroImages }) {
  ...
  const [form, setForm] = useState({ name: "", phone: "", address: "", notes: "", mode: "Delivery" });
  ...
  const submitOrder = async () => {
    if (!form.name.trim() || !form.phone.trim() || (form.mode === "Delivery" && !form.address.trim())) return;
    setSubmitting(true);
    const order = {
      id: genId(),
      ...
      total: cartTotal,
      customer: { name: form.name.trim(), phone: form.phone.trim(), address: form.address.trim(), notes: form.notes.trim(), mode: form.mode },
      status: "pending",
      createdAt: Date.now(),
    };
      ...
      {checkoutOpen && (
        <div className="fixed inset-0 z-40 flex items-center justify-center p-4">
          ...
          <div className="relative bg-green-950 border border-green-900 rounded-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            ...
            <button
              disabled={submitting || !form.name.trim() || !form.phone.trim() || (form.mode === "Delivery" && !form.address.trim())}
              onClick={submitOrder}
